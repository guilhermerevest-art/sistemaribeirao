-- Açougue Ribeirão — seed expandido (açougue + empório).
-- 12 categorias, ~50 produtos, 24 clientes, pedidos diversificados por frequência,
-- 4 ofertas (3 semana + 1 relâmpago), 6 resgates.
...
on conflict (id) do update set label = excluded.label, icone = excluded.icone, ordem = excluded.ordem;

-- ============================================================
-- Produtos (~50)
-- Os IDs seguem o padrão p-<slug> pra casar com o código atual.
-- ============================================================
...
on conflict (id) do update set
  ...
  peso_medio_peca = excluded.peso_medio_peca,
  imagem = excluded.imagem,
  percentual_cashback = excluded.percentual_cashback,
  preparos_disponiveis = excluded.preparos_disponiveis,
...
on conflict (id) do update set nome = excluded.nome, custo_pontos = excluded.custo_pontos, imagem = excluded.imagem, ativo = excluded.ativo;

-- ============================================================
-- Clientes (24) + Pedidos (60-90, distribuídos por padrão de frequência)
-- 5 fieis, 8 ocasionais, 4 novos, 4 em risco, 3 inativos — igual ao seed original.
-- Nomes brasileiros comuns, telefones DDD 34 (Uberlândia/MG).
-- Para manter o arquivo legível, geramos via DO block com random determinístico.
-- ============================================================
do $$
declare
  nomes text[] := array[
...
begin
  for i in 1..24 loop
    ...
    -- Limpa pedidos anteriores desse cliente (idempotência da seed).
    delete from public.pedidos where cliente_id = cid;
    ...
    if padrao = 'novo' then
      ...
      values (lpad((seq + 1)::text, 4, '0'), cid, itens, subtot, subtot, cb, pts, 'entregue', 'balcao', 'pix', now() - ((1 + (random()*6)::int) || ' days')::interval);
      update public.clientes set saldo_cashback = round(subtot * 0.035, 2), cashback_expira_em = now() + ((55 + (random()*5)::int) || ' days')::interval, pontos = pts, pontos_acumulado_total = pts where id = cid;
      ...
      -- 8 pedidos espaçados a cada ~7-12 dias, últimos 3 dentro de 30 dias
      for j in 0..7 loop
...
-- Atualiza saldo de cashback dos fieis (média de pedidos últimos 60 dias).
...
-- ============================================================
-- Movimento da bancada (3 novo + 1 preparando)
-- ============================================================
